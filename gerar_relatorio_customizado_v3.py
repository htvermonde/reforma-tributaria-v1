...
def _get_desc_cfop(cfop):
    ...
    resultado = DF_BASE_CFOP[
        DF_BASE_CFOP["Codigo CFOP"].astype(str).str.strip() == cfop_str
    ]
...
def get_tipi_aplicavel(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"ipi_cst": ipi_cst, "tipi": tem_tipi}
    ...
    for idx, nota in enumerate(notas, 1):
        ...
        for item_idx, item in enumerate(items):
            ...
            info_produto = {
                ...
                "DESC CST ICMS": _get_desc_cst_icms(
                    item.get("ICMS_CST") or item.get("ICMS_CSOSN", "")
                ),
                ...
                "DIFAL interestadual": identificar_difal(nota, item)
                .get("debug", {})
                .get("interestadual", False),
                "DIFAL consumidor final": identificar_difal(nota, item)
                .get("debug", {})
                .get("consumidor_final", ""),
                "DIFAL ind_ie_dest": identificar_difal(nota, item)
                .get("debug", {})
                .get("ind_ie_dest", ""),
                "DIFAL cfop": identificar_difal(nota, item)
                .get("debug", {})
                .get("cfop", ""),
...
def gerar_relatorio_customizado(ARQUIVO_JSON, ARQUIVO_EXCEL, ARQUIVO_BASE_CFOP):
    global DF_BASE_CFOP
    print("=" * 80)
